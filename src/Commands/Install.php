...
namespace Tyondo\Team\Commands;

use Illuminate\Support\Facades\Artisan;
use Illuminate\Support\Facades\File;
use Exception;

class Install extends TeamCommand
{
    /**
     ...
     * @var string
     */
    protected $signature = 'team:install {--y|y : Skip question?} {--views : Also publish tyondo team views.} {--f|force : Overwrite existing files.}';
     ...
     * @var string
     */
    protected $description = 'Tyondo team install wizard';
    ...
    public function handle()
    {
            ...
            $this->comment(PHP_EOL.'Welcome to the Tyondo team Install Wizard! You\'ll be up and running in no time...');

            try {
                $this->comment(PHP_EOL.'Publishing assets...');
                // Publish config files...
                Artisan::call('team:publish:config', [
                    '--y' => true,
                    '--force' => true,
                ]);
                $this->progress(1);
                ...
                Artisan::call('team:publish:assets', [
                    '--y' => true,
                    '--force' => true,
                ]);
                $this->progress(2);
                ...
                Artisan::call('team:publish:packages', [
                    '--y' => true,
                    '--force' => false,
                ]);
                $this->progress(3);
                ...
                // Publish view files...
                if ($withViews) {
                    Artisan::call('team:publish:views', [
                        '--y' => true,
                        '--force' => $force,
                    ]);
                }
                ...
                $this->info(PHP_EOL.'Adding team routes to routes/web.php');
                $this->progress(5);
                File::append(
