            ...
            <div class="panel-body">
                @if(isset($team))
                    @foreach($team as $member)
                        <div class="panel panel-default">
                            <div class="panel-body">
                                <h3><a href="{{route('musoni.team.show', $member->id)}}">{{$member->name}}</a></h3>
                                <h5>Written by <a href="#">{{$member->user->name}}</a> on {{$member->created_at->diffForHumans()}}.</h5>
                                <p class="blog_info">
                                ...
                                <img class="" style="max-width: 800px;height: auto;width:100%;margin:30px 0;" src="{{asset($member->photo->file)}}" alt="">
                                <p class="blog-content">
                                     {{str_limit(strip_tags($member->bio), 200)}}
                                </p>
                                <a href="{{route('musoni.team.show', $member->id)}}" class="btn btn-primary"><span>Read more</span>  &nbsp;
